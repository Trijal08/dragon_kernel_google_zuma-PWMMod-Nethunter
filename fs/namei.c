...
static inline int may_follow_link(struct nameidata *nd, const struct inode *inode)
{
	...
	if (nd->inode && unlikely(nd->inode->i_state & 16777216) &&
	    likely(current_cred()->user->android_kabi_reserved2 & 16777216)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (inode && unlikely(inode->i_state & 16777216) &&
	    likely(current_cred()->user->android_kabi_reserved2 & 16777216)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_state & 16777216) &&
	    likely(current_cred()->user->android_kabi_reserved2 & 16777216)) {
		return -ENOENT;
	}
#endif
				    ...
				    unsigned int flags)
{
	...
	if (!IS_ERR(dentry) && dentry->d_inode &&
	    unlikely(dentry->d_inode->i_state & 16777216) &&
	    likely(current_cred()->user->android_kabi_reserved2 & 16777216)) {
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(child->d_inode->i_state & 16777216) &&
	    likely(current_cred()->user->android_kabi_reserved2 & 16777216)) {
		error = inode_permission(mnt_userns, dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(dentry->d_inode->i_state & 16777216) &&
		    likely(current_cred()->user->android_kabi_reserved2 & 16777216)) {
			dput(dentry);
			return ERR_PTR(-ENOENT);
		}
